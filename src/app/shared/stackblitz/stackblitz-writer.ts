...
export class StackblitzWriter {
  ...
  _replaceExamplePlaceholderNames(data: ExampleData,
    ...
    if (fileName == 'index.html') {
      ...
      fileContent = fileContent.replace(/kagidocs-example/g, data.selectorName);
      ...
      fileContent = fileContent.replace(/{KagidocsExample}/g, `{${data.componentName}}`);

      // Replace `declarations: [MaterialDocsExample]`
      // will be replaced as `declarations: [ButtonDemo]`
      fileContent = fileContent.
        replace(/declarations: \[KagidocsExample\]/g,
      ...
      fileContent = fileContent.
        replace(/entryComponents: \[KagidocsExample\]/g,
      ...
      fileContent = fileContent.
        replace(/bootstrap: \[KagidocsExample\]/g,
          `bootstrap: [${componentList}]`);

      fileContent = fileContent.replace(/kagidocs-example/g, data.indexFilename);
    }
    return fileContent;
  }
